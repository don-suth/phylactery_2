...
from .models import Member, RankChoices, Rank
...
@method_decorator(gatekeeper_required, name="dispatch")
class GatekeeperListView(ListView):
	model = Member
	paginate_by = 50
	template_name = "members/gatekeeper_list.html"
	
	search_query = None
	
	def get_queryset(self):
		active_gatekeeper_ranks = Rank.objects.all_active().filter(rank_name=RankChoices.GATEKEEPER)
		qs = Member.objects.filter(ranks__in=active_gatekeeper_ranks)
